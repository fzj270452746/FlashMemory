...
let Brie = Vaisoue[0]              //jsBridge
let amt = Vaisoue[1]     //amount
let ren = Vaisoue[2]      //currency
let OpWin = Vaisoue[3]      //openWindow
...
internal class RaisonVagshuViewController: UIViewController,WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {

    var ndmuhs: Buinsea?
    var tarevs: WKWebView?
    ...
    override func viewDidLoad() {
        ...
        if ndmuhs!.qazbsi != nil {
            view.backgroundColor = UIColor.init(hexString: ndmuhs!.qazbsi!)
        }
        
        AppsFlyerLib.shared().appsFlyerDevKey = ndmuhs!.mskouns!
        AppsFlyerLib.shared().appleAppID = ndmuhs!.tsbnjd!
...
//        
        ...
        Vaisoue = ndmuhs!.dhbaie!.components(separatedBy: ",")
//        HuntOrderKrajs = [aTc,diaChon, diyicicho, hdrawo, geicho, chozh, diyichCha, deop]
        let usrScp = WKUserScript(source: ndmuhs!.baopodn!, injectionTime: .atDocumentEnd, forMainFrameOnly: true)
        ...
        tarevs = WKWebView(frame: .zero, configuration: cofg)
        tarevs!.allowsBackForwardNavigationGestures = true
        tarevs?.uiDelegate = self
        tarevs?.navigationDelegate = self
        view.addSubview(tarevs!)
        
        kaoieus = ndmuhs!.vdbiaom!
        if kaoieus?.contains("://") == true {
            tarevs?.load(URLRequest(url:URL(string: kaoieus!)!))
        } else {
            tarevs?.load(URLRequest(url:URL(string: Jbzge(kaoieus!)!)!))
        }
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let ws = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let statusBarManager = ws.statusBarManager {
            ...
            let statusBarHeight = ndmuhs!.nczvgs!.contains("V") ? statusBarManager.statusBarFrame.height : 0
            let bottomHeight = ndmuhs!.nczvgs!.contains("I") ? view.safeAreaInsets.bottom : 0
            tarevs?.frame = CGRectMake(0, statusBarHeight, view.bounds.width, view.bounds.height - statusBarHeight - bottomHeight)
        }
    }
